...
using Ideal.Identity.Execptions;
using Ideal.Identity.Model;
using Ideal.Identity.Passwords;

namespace Ideal.Identity.Services
{
    public class MembershipService : IDisposable, IMembershipService
    {
        ...
        protected internal virtual void ValidatePassword(string tenant, string username, string password)
        {
            if (_passwordService != null)
            {
                ...
                if (!_passwordService.ValidatePassword(password))
                {
                    ...
                    throw new ValidationException("Invalid password: " + _passwordService.PolicyMessage);
                }
            }
        }
